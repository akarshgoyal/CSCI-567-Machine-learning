...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    p=len(z)
    q=np.ones((p,1))
    r=np.concatenate((q,X),axis=1)
    s=np.insert(w,0,b)
    
    for _ in range(max_iterations):
        t=sigmoid(np.dot(s,r.transpose()))-y
        u=step_size*np.dot(t,r)/N
        s=s-u
...
def multinomial_train(X, y, C, 
    ...
    for j in range(0,max_iterations):
        ...
        x_=x-max_row.reshape(-1,1)
        summa=np.sum(np.exp(x_),axis=1).reshape(-1,1)
        p=np.exp(x_)/summa
        gradient=np.matmul((p-mat).T,r)
        s=s-step_size/N*gradient
...
def multinomial_predict(X, w, b):
    ...
    r=np.concatenate((q,X),axis=1)
    #s=np.zeros((C,D+1))
    #s=np.concatenate((b,w),axis=1)
    b=np.array([b]).T
    s=np.append(b,w, axis=1)
    t=np.dot(s,r.transpose())
    per=t
    #print(per.shape)
    #for i in range(len(per)):
    preds=np.argmax(t,axis=0)
    #print(preds.shape)
    ...
    for i in range(C):
        ...
        for j in range(len(y)):
            if(i==l[j]):
                ...
                l[j]=0
        w[i,:], b[i] = binary_train(X, l)
        """
        for _ in range(max_iterations):
                t=sigmoid(np.dot(s[i],r.transpose()))-l
                u=step_size*np.dot(t,r)/N
                s[i]=s[i]-u
        """
